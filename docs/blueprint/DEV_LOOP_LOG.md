...
## Loop-143 / 外部工具 metadata 入库边界收敛 - 2026-06-18

### 本轮目标

1. 继续审查外部工具网关，处理配置 metadata 和调用审计 metadata 入库前仍忽略 JSON marshal 错误的问题。
2. 避免外部工具调用链路或未来 metadata 扩展携带不可序列化、非有限数或异常放大的值时，静默写入空值/坏值。
3. 将外部工具配置 metadata、审计 metadata 的 JSON 字节预算固化进静态验收。

### 代码交付

1. `backend/internal/platform/externaltool/store.go` 新增 `maxExternalToolConfigMetadataJSONBytes` 和 `maxExternalToolAuditMetadataJSONBytes`。
2. 新增 `marshalExternalToolMetadataJSON()`，统一处理 nil metadata、JSON marshal 错误和字节上限，错误归一为 `ErrInvalidRequest`。
3. `UpsertConfig()` 和 `recordAudit()` 改为进入租户事务前生成已校验 metadata JSON，不再忽略 `json.Marshal(normalized.Metadata)` / `json.Marshal(input.Metadata)` 错误。
4. `backend/internal/platform/externaltool/store_test.go` 新增不可 JSON 值、NaN、超预算 metadata 和 nil metadata 归一化回归测试。
5. `infra/scripts/acceptance_tail_check.py --static-docs` 增加外部工具 metadata JSON 边界防回退检查。
...
## Loop-155 / 造价任务 JSON 读取边界收敛 - 2026-06-18

### 本轮目标

1. 继续审查造价 AI 任务读取链路，处理 `scanTask()` 读取 `payload`、`route`、`result` 时仍忽略 `json.Unmarshal` 错误的问题。
2. 避免异常存储 JSON 类型、损坏 JSON 或超预算任务结果静默变成空对象，影响成本建议任务详情和审计排查。
3. 将造价任务 JSON 读取边界固化进静态验收。

### 代码交付

1. `backend/internal/platform/cost/store.go` 新增 `unmarshalCostTaskJSON()`，空值和 `null` 归一化为 `{}`，并按既有 payload/route/result 字节预算校验读取侧 JSON。
2. `scanTask()` 改为显式处理 `payload`、`route`、`result` 解析错误，不再忽略 `json.Unmarshal` 失败。
3. `backend/internal/platform/cost/store_test.go` 新增 fake scanner，覆盖坏 payload/route/result 被拒绝，以及空/null 存储值归一化为 `{}`。
4. `infra/scripts/acceptance_tail_check.py --static-docs` 增加造价任务 JSON 读取边界防回退检查。
...
1. 本轮治理的是 Go 造价任务读取可靠性，没有新增成本建议算法、AI Provider 或前端任务详情视图。
2. 读取侧沿用既有写入侧字节预算；如果后续成本建议结果需要保存更大结构，应改为文件资产或分表明细，而不是扩大任务 JSON。

## Loop-156 / 知识库任务 JSON 读取边界收敛 - 2026-06-18

### 本轮目标

1. 继续审查知识库 AI 任务读取链路，处理 `scanTask()` 读取 `payload`、`route`、`result` 时仍忽略 `json.Unmarshal` 错误的问题。
2. 避免异常存储 JSON 类型、损坏 JSON 或超预算任务结果静默变成空对象，影响知识库文档处理任务详情、回调结果和审计排查。
3. 将知识库任务 JSON 读取边界固化进静态验收。

### 代码交付

1. `backend/internal/platform/knowledge/store.go` 新增 `unmarshalKnowledgeTaskJSON()`，空值和 `null` 归一化为 `{}`，并按既有 payload/route/result 字节预算校验读取侧 JSON。
2. `scanTask()` 改为显式处理 `payload`、`route`、`result` 解析错误，不再忽略 `json.Unmarshal` 失败。
3. `backend/internal/platform/knowledge/store_test.go` 新增 fake scanner，覆盖坏 payload/route/result 被拒绝，以及空/null 存储值归一化为 `{}`。
4. `infra/scripts/acceptance_tail_check.py --static-docs` 增加知识库任务 JSON 读取边界防回退检查。
...
1. 本轮治理的是 Go 知识库任务读取可靠性，没有新增文档解析算法、AI Provider 或前端任务详情视图。
2. 读取侧沿用既有写入侧字节预算；如果后续知识库处理结果需要保存更大结构，应改为文件资产或分表明细，而不是扩大任务 JSON。

## Loop-157 / AI 调用审计任务 JSON 读取边界收敛 - 2026-06-18

### 本轮目标

1. 继续审查 AI 调用审计链路，处理 `RecordTaskCallback()` 读取任务 `route`、`result` 时仍忽略 `json.Unmarshal` 错误的问题。
2. 避免异常存储 JSON 静默进入审计记录，影响 provider/model、token、estimated_cost、fallback 和业务引用的真实性。
3. 将 AI 调用审计任务 JSON 读取边界固化进静态验收。

### 代码交付

1. `backend/internal/platform/aicall/store.go` 新增 `unmarshalAITaskJSON()`，空值和 `null` 归一化为 `{}`，并按 route/result 字节预算校验读取侧 JSON。
2. `RecordTaskCallback()` 改为显式处理 `route` 解析错误；`result` 在没有本次 callback result 时才回退解析存储值，避免旧存储 result 损坏阻断新回调结果入审计。
3. `backend/internal/platform/aicall/pricing_test.go` 覆盖坏任务 JSON 被拒绝、空/null 存储值归一化，以及 callback result 优先于旧存储 result。
4. `infra/scripts/acceptance_tail_check.py --static-docs` 增加 AI 调用审计任务 JSON 读取边界防回退检查。
...
## Loop-161 / AI 调用审计 biz_ref 读取边界收敛 - 2026-06-18

### 本轮目标

1. 继续审查 AI 调用审计读取链路，处理 `scanLog()` 读取 `biz_ref` 时仍忽略 `json.Unmarshal` 错误的问题。
2. 避免损坏、错形或超预算 `biz_ref` 静默进入 AI 调用归属、成本审计和团队页展示。
3. 将 AI 调用审计 `biz_ref` 读取边界固化进静态验收。

### 代码交付

1. `backend/internal/platform/aicall/store.go` 新增 `unmarshalRecordBizRef()`，读取侧复用既有 `biz_ref` 字节预算和 `external_task_id` 长度约束。
2. `scanLog()` 改为先返回底层 `row.Scan` 错误，再显式处理 `biz_ref` 解析错误，不再忽略 `json.Unmarshal` 失败。
3. `mapFromMap()` 改为复用 `unmarshalAITaskJSON()` 做 JSON 兼容转换，不再吞掉内部 `json.Unmarshal` 错误。
4. `backend/internal/platform/aicall/pricing_test.go` 新增 fake scanner 回归测试，覆盖坏 `biz_ref` 被拒绝、空/null 存储值归一化为 `{}`，以及兼容转换的非对象值降级为空对象。
5. `infra/scripts/acceptance_tail_check.py --static-docs` 增加 AI 调用审计 `biz_ref` 读取边界防回退检查。
