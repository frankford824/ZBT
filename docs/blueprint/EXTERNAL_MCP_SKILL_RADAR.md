...
8. `/tenders` 的“外部标讯”页签已接入 Handaas 招投标搜索：只发送关键词、地区和发布日期范围，结果归一后可保存为租户内 `external_mcp` 标讯来源。

继续增强清单：

1. Provider 生产验证：用真实 Handaas、AutoRFP、qlows 或 Loopio 凭证跑只读 smoke test，并记录工具名与响应结构。
2. 业务入口：继续把企业画像、外部应答库检索结果归一为 `external_mcp` 来源。
3. 禁止首批工具接收完整招标文件、投标文件、报价明细和合同正文。

### P1：业务入口

1. 标讯大厅：根据行业、地区、关键词检索潜在标讯，当前已先接 Handaas 公开标讯搜索。
