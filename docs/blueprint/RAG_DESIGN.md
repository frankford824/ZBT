...
当前实现状态：text/plain、PDF 和 Word 已具备最小文本抽取、切片和 knowledge_chunks 入库能力，`POST /knowledge/search` 已提供租户内关键词检索并返回 source_refs。章节重新生成会保存 AI 返回的 source_refs，并将可解析或未解析引用写入 knowledge_references；向量 embedding、pgvector 召回、RRF、rerank 和真实检索增强生成仍待实现。
