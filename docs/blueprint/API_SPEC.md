...
标书一期已落地最小 bid_documents、bid_parts、bid_chapters、bid_exports 数据闭环。`GET /bid-templates` / `POST /bid-templates/:templateId/use` 已接入 `bid_templates` RLS 表，前端 `/bids/templates` 从真实模板库读取分类、版本、章节结构和使用次数，并可一键按模板创建草稿标书。`POST /bids/:id/exports` 支持 `export_type=docx` 或 `zip`。docx 导出时 Go 写入 ai_tasks 和待确认 file_asset 后调用 Python `/tasks/export/docx`；ZIP 打包时 Go 汇总技术标/商务标内容后调用 Python `/tasks/export/zip`。Python 生成文件并上传 MinIO，再通过 HMAC 回调 Go，Go 将 bid_exports 和 file_assets 标记为 ready/done。`GET /bid-exports/:exportId` 在导出完成后返回下载预签名 URL。
