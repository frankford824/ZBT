...
招标文件解析和目录大纲已落地 `POST /bids/:id/upload-tender-file`、`POST /bids/:id/parse-tender`、`GET/PUT /bids/:id/parse-result`、`GET /bids/:id/requirements`、`GET /bids/:id/requirements/export`、`GET /bids/:id/pipeline-gates`、`POST /bids/:id/outline/generate`、`GET/PUT /bids/:id/parts/:partId/outline` 和 `GET/PUT /bids/:id/material-selection`。上传链路复用 `file_assets` 私有 MinIO 预签名上传，绑定后写入 `bid_tender_files` 和 `bid_parse_results`；解析结果会同步要求矩阵，要求矩阵导出返回 UTF-8 CSV，包含覆盖状态、响应证据、响应来源和招标原文来源；解析阶段闸门会随上传、解析、回调和人工确认更新。目录生成前必须满足 `interpret=passed`，通过后生成章节大纲并写入 `plan=passed`。整标生成前必须满足 `plan=passed`，导出前必须满足 `generate=passed` 和 `check=passed`；生成、合规和导出会继续更新 `generate`、`check`、`format` 阶段状态。前端 7 步向导的 1-4 步已接入真实上传、解析确认、目录生成/编辑和素材选择接口。

章节一期已落地 `PUT /chapters/:chapterId/content`、`POST /chapters/:chapterId/accept`、`POST /chapters/:chapterId/regenerate`、`GET /chapters/:chapterId/versions`、`GET /chapters/:chapterId/diff`、`GET /bids/:id/generation/stream` 和 `GET /bids/:id/generation-coverage`。保存和采纳会同步写入 `bid_chapter_versions`；重新生成由 Go 先检索当前租户 `knowledge_chunks`，创建 `ai_tasks` 并把章节置为 `generating`，再调用 Python `/tasks/chapter-generate`。Python 返回 202 + task_id 后在后台走 ModelRouter，生成完成后通过 HMAC 回调 Go，Go 回写 Tiptap JSON、source_refs、needs_human_input、版本快照和 `knowledge_references`；真实 chunk 引用会解析为 `source_document_id` + `chunk_id`。生成回调中的 `self_check.requirement_coverage` 会同步回写 `bid_requirement_items.coverage_status` / `needs_review`，并把响应侧证据保存到 `metadata.latest_coverage`；前端“响应要点”表会消费该字段展示证据摘要和来源数量。`GET /bids/:id/generation-coverage` 会把当前标书的 `bid_requirement_items`、最新章节版本 `model_metadata.self_check.requirement_coverage`、章节 `source_refs` 和已解析 `knowledge_chunks` 组合成生成覆盖评测 JSON。前端通过带 Authorization 头的 fetch SSE 订阅 `/bids/:id/generation/stream` 获取章节与 task 快照，并保留 `GET /ai-tasks/:taskId` 轮询兜底。
