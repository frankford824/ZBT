...
招标文件解析和目录大纲已落地 `POST /bids/:id/upload-tender-file`、`POST /bids/:id/parse-tender`、`GET/PUT /bids/:id/parse-result`、`GET /bids/:id/requirements`、`GET /bids/:id/pipeline-gates`、`POST /bids/:id/outline/generate`、`GET/PUT /bids/:id/parts/:partId/outline` 和 `GET/PUT /bids/:id/material-selection`。上传链路复用 `file_assets` 私有 MinIO 预签名上传，绑定后写入 `bid_tender_files` 和 `bid_parse_results`；解析结果会同步要求矩阵，解析阶段闸门会随上传、解析、回调和人工确认更新。目录生成前必须满足 `interpret=passed`，通过后生成章节大纲并写入 `plan=passed`。整标生成前必须满足 `plan=passed`，导出前必须满足 `generate=passed` 和 `check=passed`；生成、合规和导出会继续更新 `generate`、`check`、`format` 阶段状态。前端 7 步向导的 1-4 步已接入真实上传、解析确认、目录生成/编辑和素材选择接口。
