...
工程1运行态验收使用同一批真实文件走 HTTP API 全链路，需先启动本地 Docker 栈：
...
该脚本会上传 `采购文件桥梁检查.pdf` 作为招标文件，解析并确认响应矩阵；处理 `响应文件格式.docx` 和 `清单（固化）(1).xlsx` 为知识文档；随后全量生成章节，检查生成覆盖来源引用，执行合规检查并通过现有忽略建议流程处理非阻断 warn，最后导出 DOCX。
