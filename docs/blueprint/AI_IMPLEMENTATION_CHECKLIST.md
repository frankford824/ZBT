...
- `ai-service/app/gateway/model_router.py` 已支持按 `AI_MODEL_PRICING_JSON` 或配置内 `pricing` 估算 AI 调用费用；Python AI 服务会在 tender_parse 六模块、knowledge embedding、chapter_generate、chapter_action 和 cost_advice 回调中写出 `estimated_cost` 和运行期 quota 快照，同步 `/embeddings/knowledge` 与 `/rerank/knowledge` 响应也会返回 `token_usage`、`estimated_cost` 和 `quota_usage`；Go 后端写入 `ai_call_logs` 时优先使用 AI 服务返回的 token/费用，并保留本地估算兜底。
- `ai-service/app/evaluation/provider_canary_eval.py` 已提供真实 Provider canary。默认覆盖 `chapter_generate`、`knowledge_embedding`、`knowledge_rerank`，本地无密钥时可 `--allow-skip`，生产验收应使用 `--strict --call-provider --require-cost`，确认非 Mock 路由、Provider 健康、最小真实调用、正向费用估算和 quota 快照。
...
- `backend/internal/platform/bid/store.go` 已提供 `GET /bids/:id/generation-coverage` 运行态导出：从 `bid_requirement_items`、最新章节版本、章节 `source_refs` 与已解析 `knowledge_chunks` 组合出可直接交给离线评测器的 JSON，并默认携带 mandatory 覆盖、source_ref 解析、引用号/定位码和来源位置完整率阈值；核心验收脚本会检查导出中存在要求项、覆盖矩阵行和可追溯的覆盖来源。
