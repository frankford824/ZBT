...
- `ai-service/app/gateway/model_router.py` 已支持按 `AI_MODEL_PRICING_JSON` 或配置内 `pricing` 估算 AI 调用费用；Python AI 服务会在 tender_parse 六模块、knowledge embedding、chapter_generate、chapter_action 和 cost_advice 回调中写出 `estimated_cost` 和运行期 quota 快照，Go 后端仍保留落库价格表兜底。
