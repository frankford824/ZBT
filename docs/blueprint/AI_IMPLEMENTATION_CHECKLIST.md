...
3. AutoRFP 式“问题矩阵/响应矩阵”已形成运行态闭环：招标要求可落入独立表，章节生成可回写覆盖状态、响应证据和来源数量，人工可调整覆盖状态和补充证据，支持批量标记覆盖状态，单条要求可查看模型/人工覆盖历史，并可导出评审响应矩阵 CSV 和带覆盖历史工作表的 xlsx；仍需继续补批量证据选区和高级筛选。
...
- `backend/internal/api/routes.go` 已提供 `PATCH /bids/:id/requirements`，前端“响应要点”表支持勾选多条要求后批量标记覆盖状态；批量操作会逐条写入覆盖历史，任一要求项不存在时整批失败。
...
2. 响应矩阵页：
   ...
   - 章节覆盖状态、来源状态、合规状态。
   - 当前已落地：响应要点表展示覆盖状态、证据摘要和来源数量，支持人工调整覆盖状态、批量标记覆盖状态、补充证据，并支持导出评审响应矩阵 CSV 和带历史工作表的 xlsx。
