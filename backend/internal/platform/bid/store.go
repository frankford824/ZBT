...
func scanTask(row scanner) (Task, error) {
	...
	if task.Payload, err = unmarshalBidTaskJSON(payloadRaw, maxBidTaskPayloadJSONBytes); err != nil {
		return Task{}, err
	}
	if task.Route, err = unmarshalBidTaskJSON(routeRaw, maxBidTaskRouteJSONBytes); err != nil {
		return Task{}, err
	}
	if task.Result, err = unmarshalBidTaskJSON(resultRaw, maxBidTaskResultJSONBytes); err != nil {
		return Task{}, err
	}
	return task, err
}
...
func unmarshalBidTaskJSON(raw []byte, maxBytes int) (map[string]any, error) {
	result := map[string]any{}
